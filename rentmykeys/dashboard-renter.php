...
// Get renter's vehicles
$vehicles_sql = "SELECT v.*, COUNT(DISTINCT b.booking_id) as bookings_count 
                FROM vehicles v 
...
// Get recent bookings for renter's vehicles
$bookings_sql = "SELECT b.*, v.title, v.vehicle_type, u.full_name as customer_name, u.profile_picture as customer_picture 
                FROM bookings b 
...
$active_bookings_sql = "SELECT COUNT(*) as count 
                        FROM bookings b 
                        JOIN vehicles v ON b.vehicle_id = v.vehicle_id 
                        WHERE v.user_id = ? AND b.status IN ('pending', 'confirmed')";
$stmt = $conn->prepare($active_bookings_sql);
$stmt->bind_param("i", $user_id);
$stmt->execute();
$result = $stmt->get_result();
...
// Total earnings
$earnings_sql = "SELECT SUM(b.total_price) as total 
                FROM bookings b 
                JOIN vehicles v ON b.vehicle_id = v.vehicle_id 
                WHERE v.user_id = ? AND b.status = 'completed'";
...
<head>
    ...
    <title>Renter Dashboard - RentMyKeys</title>
    <!-- Tailwind CSS CDN -->
                ...
                <div class="flex items-center">
                    ...
                    <div class="relative" x-data="{ open: false }">
                        ...
                        <div x-show="open" @click.away="open = false" class="absolute right-0 mt-2 w-48 glass-card rounded-md shadow-lg py-1 z-50" style="display: none;">
                            <a href="dashboard-renter.php" class="block px-4 py-2 text-sm text-white hover:bg-gray-800/50 transition duration-300">Dashboard</a>
    ...
    <div class="relative z-10 py-6 px-4 sm:px-6 lg:px-8">
        <div class="max-w-7xl mx-auto">
            ...
            <div class="glass-card rounded-2xl p-6 mb-8" style="background: rgba(17, 25, 51, 0.7); backdrop-filter: blur(20px); border: 1px solid rgba(34, 46, 66, 0.3); box-shadow: 0 8px 32px rgba(0, 0, 0, 0.3);">
                <div class="flex flex-col md:flex-row items-center">
                    <div class="md:w-3/4">
                        ...
                        <p class="text-gray-500">Manage your vehicles, bookings, and earnings all in one place.</p>
                    </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                <div class="glass-card rounded-xl p-6 stats-card">
                    <div class="flex items-center">
                        ...
                        <div>
                            ...
                            <h3 class="text-2xl font-bold gradient-text"><?php echo $total_vehicles; ?></h3>
                        </div>
                    </div>
                </div>
                ...
                <div class="glass-card rounded-xl p-6 stats-card">
                    <div class="flex items-center">
                        <div class="bg-primary/20 rounded-full p-3 mr-4">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-8 w-8 text-primary" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5a2 2 0 00-2 2v12a2 2 0 002 2z" />
                            </svg>
                        </div>
                        <div>
                ...
                <div class="glass-card rounded-xl p-6 stats-card">
                    <div class="flex items-center">
                        ...
                        <div>
                            ...
                            <h3 class="text-2xl font-bold gradient-text">₹<?php echo number_format($total_earnings, 2); ?></h3>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="glass-card rounded-2xl p-6 mb-8">
                ...
                <?php if (empty($recent_bookings)): ?>
                    <div class="text-center py-8">
                        ...
                        <p class="text-gray-400 mt-4">No bookings yet. Start by adding vehicles to your listing.</p>
                        <a href="add-vehicle.php" class="gradient-border inline-block mt-4">
                                        ...
                                        <td class="py-4">
                                            <div class="flex items-center">
                                                <div class="gradient-border rounded-full h-8 w-8 inline-block">
                                                    <img src="<?php echo $booking['customer_picture'] ?? 'assets/images/default-avatar.png'; ?>" class="h-8 w-8 rounded-full object-cover" alt="Customer">
                                                </div>
                                                <span class="ml-2"><?php echo $booking['customer_name']; ?></span>
                                            </div>
                                        </td>
                                        <td class="py-4">
                                            <div class="text-sm">
            ...
            <div class="glass-card rounded-2xl p-6">
                <div class="flex justify-between items-center mb-6">
                    <h2 class="text-xl font-semibold gradient-text">My Vehicles</h2>
                    <a href="my-vehicles.php" class="text-primary hover:text-blue-400 transition flex items-center action-button px-3 py-1 rounded-full">
                        View All
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 ml-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" />
                        </svg>
                ...
                <?php if (empty($vehicles)): ?>
                    <div class="text-center py-8">
                        ...
                        <p class="text-gray-400 mt-4">You haven't added any vehicles yet.</p>
                        <a href="add-vehicle.php" class="gradient-border inline-block mt-4">
                            <button class="bg-gradient-to-r from-primary to-blue-500 hover:from-blue-600 hover:to-blue-700 text-white font-bold py-2 px-4 rounded-lg transition duration-300 ease-in-out flex items-center">
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4" />
                ...
                <?php else: ?>
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        <?php foreach (array_slice($vehicles, 0, 6) as $vehicle): ?>
                            <?php 
                                // Get primary image
                                $image = isset($vehicle['image_url']) ? $vehicle['image_url'] : 'assets/images/vehicle-placeholder.jpg';
                                if (!isset($vehicle['image_url'])) {
                                    // Get primary image from vehicle_images table if available
                                    $img_sql = "SELECT image_url FROM vehicle_images WHERE vehicle_id = ? AND is_primary = 1 LIMIT 1";
                                    $img_stmt = $conn->prepare($img_sql);
                                    $img_stmt->bind_param("i", $vehicle['vehicle_id']);
                                    $img_stmt->execute();
                                    $img_result = $img_stmt->get_result();
                                    if ($img_result->num_rows > 0) {
                                        $img_row = $img_result->fetch_assoc();
                                        $image = $img_row['image_url'];
                                    }
                                }
                            ?>
                            <div class="glass-card rounded-xl overflow-hidden vehicle-card">
                                <a href="vehicle-details.php?id=<?php echo $vehicle['vehicle_id']; ?>" class="block relative aspect-[16/9] overflow-hidden">
                                    <img src="<?php echo $image; ?>" alt="<?php echo $vehicle['title']; ?>" class="w-full h-full object-cover">
                                    <div class="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black to-transparent p-4">
                                        ...
                                        <?php if (!$vehicle['availability']): ?>
                                            <div class="px-2 py-1 bg-red-500 text-white text-xs font-semibold rounded-full inline-block ml-2">
                                                Unavailable
                                            </div>
                                        <?php endif; ?>
                                    </div>
                                ...
                                <div class="p-4">
                                    ...
                                    <div class="flex flex-wrap text-xs text-gray-400 mt-2 space-x-2">
                                        ...
                                        <span class="flex items-center bg-gray-800/50 px-2 py-1 rounded-full">
                                            ...
                                            <?php echo $vehicle['bookings_count']; ?> Bookings
                                        </span>
                                        <?php if (isset($vehicle['transmission']) && $vehicle['transmission']): ?>
                                            <span class="flex items-center bg-gray-800/50 px-2 py-1 rounded-full">
                                                <svg xmlns="http://www.w3.org/2000/svg" class="h-3 w-3 mr-1 text-primary" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    ...
                                    <div class="mt-4 flex space-x-2">
                                        <a href="edit-vehicle.php?id=<?php echo $vehicle['vehicle_id']; ?>" class="bg-gray-800 hover:bg-gray-700 text-white text-sm font-medium py-2 px-4 rounded-lg transition duration-300 ease-in-out flex-1 text-center">
                                            Edit
                                        </a>
                                        <a href="vehicle-details.php?id=<?php echo $vehicle['vehicle_id']; ?>" class="bg-gradient-to-r from-primary to-blue-500 hover:from-blue-600 hover:to-blue-700 text-white text-sm font-medium py-2 px-4 rounded-lg transition duration-300 ease-in-out flex-1 text-center">
                                            View
                                        </a>
                                    </div>
                                </div>
                    ...
                    <?php if (count($vehicles) > 6): ?>
                        <div class="text-center mt-6">
                            <a href="my-vehicles.php" class="text-primary hover:text-blue-400 transition inline-flex items-center bg-gray-800/50 px-4 py-2 rounded-full action-button">
                                View All Vehicles
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 ml-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" />
                                </svg>
